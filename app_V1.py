...
def clear_all():
    for key in [
        "logged_in", "user", "msme_df", "parsed_data", "uploaded_file", "uploaded_msme"
    ]:
...
# Add Refresh (Clear All) button at the top
st.button("🔄 Refresh / Start New Upload", on_click=clear_all)
...
""")
    ...
    # Inline edit using data_editor with dropdowns
    st.markdown("**Edit MSME mapping (inline)** — Edit here. Any supplier included below and left blank will be treated as Non MSME registered. If you don't know the MSME status of the supplier and want it to be treated as Non MSME, leave blank.")

    # Dropdown options for MSME columns
    msme_options = {
        "Registered (Yes/No)": ["Yes", "No"],
        "Category (Micro/Small/Medium)": ["Micro", "Small", "Medium"],
        "Business Type (Trader/Manufacturer/Service Provider)": [
            "Trader", "Manufacturer", "Service Provider"
        ]
    }

    # Prepare column config for dropdowns
    column_config = {
        "Registered (Yes/No)": st.column_config.SelectboxColumn(
            "Registered (Yes/No)", options=msme_options["Registered (Yes/No)"], required=False
        ),
        "Category (Micro/Small/Medium)": st.column_config.SelectboxColumn(
            "Category (Micro/Small/Medium)", options=msme_options["Category (Micro/Small/Medium)"], required=False
        ),
        "Business Type (Trader/Manufacturer/Service Provider)": st.column_config.SelectboxColumn(
            "Business Type (Trader/Manufacturer/Service Provider)", options=msme_options["Business Type (Trader/Manufacturer/Service Provider)"], required=False
        ),
        "Supplier Name": st.column_config.TextColumn("Supplier Name", disabled=True)
    }

    edited = st.data_editor(
        st.session_state.msme_df,
        num_rows="dynamic",
        ...
        with run_col:
            if st.button("Run Processing (apply MSME exemptions)"):
                with st.spinner("Processing..."):
                    ...
                    # Prepare export only, no previews
    ...
    # Add Refresh (Clear All) button at the bottom
    st.write("---")
    st.button("🔄 Refresh / Start New Upload (bottom)", on_click=clear_all)
